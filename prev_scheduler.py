...
def get_jss_bqm(job_dict, max_time=None, stitch_kwargs=None):
    ...
    scheduler = ShiftScheduler(job_dict, max_time)
...
def get_label(shift, day):
    """Creates a standardized name for variables in the nurse satisfaction problem.
    """
    return "{shift.nurse}_{shift.day}".format(**locals())
...
class ShiftScheduler:
    def __init__(self, job_dict, max_time=None):
        ...
        # will get decremented by 1 for zero-indexing; see _process_data
    ...
    def _process_data(self, jobs):
        """Process user input into a format that is more convenient for ShiftScheduler functions.
        """
    ...
    def _edit_bqm_for_shortest_schedule(self, bqm):
        ...
        for i in self.last_task_indices:
            ...
            for t in range(self.max_time + 1):
                # -1 to get last unit of time the task occupies
